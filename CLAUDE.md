...
## 네이버 로그인 웹 OAuth 전환 (2026-08-07 저녁) — 코드 완료, 콘솔에 웹 환경 등록 필요

위 결론대로 SDK 를 걷어내고 **앱 자체 WebView + 서버 코드 교환**으로 전환했다(토스 결제창과 같은 인터셉트 패턴).
...
- **막힌 지점: 네이버 앱(ART_NARA_DEV, client_id `t0btvy…`)이 로그인 자체를 거부한다.** 동의 화면 요청이 `disp_stat=207`("ART_NARA_DEV 서비스 설정에 오류가 있어")로 거절된다. **앱 코드 문제가 아니다** — 에뮬레이터에서 WebView 가 정상적으로 뜨고 네이버까지 도달한 뒤 저 오류 화면을 받는다.
  ...
  - **→ 해결됨(2026-08-07 밤).** 새 앱(client_id `mpu67h…`)으로 바꾸자 같은 요청이 **정상 로그인 화면**을 반환한다. **기존 앱이 콘솔 단에서 망가져 있었던 게 확정** — 코드는 처음부터 정상이었다. 앱은 설정을 서버에서 받아오므로 **재빌드 없이 키 교체만으로 적용**된다.

## 네이버 로그인 전 구간 실측 완료 (2026-08-08 새벽)

에뮬레이터에서 **동의 → 인가 코드 인터셉트 → 서버 토큰 교환 → 프로필 검증 → 회원 생성 → JWT 발급 → 자동 로그인 → 역할 선택**까지 왕복 확인. 로그: `POST /auth/naver/code 200` → `네이버 로그인 성공 (isNewUser: true, userId: 41)`, 서버 `OAuth 로그인: provider=NAVER, userId=41`. 관리자 API 로 DB 저장도 확인(`id=41, nickname=Kyoung Won, email=worms213@naver.com`). 재실행 시 `자동 로그인 성공 (userId: 41)` → 역할 선택 화면.

- **막판에 걸린 진짜 버그**: 최초 네이버 가입이 500 으로 실패했다. OAuth 는 전부 성공하고 **회원 저장에서만** `users.provider` 의 `('GOOGLE','KAKAO')` CHECK 제약에 걸렸다 — 알림 type 과 **똑같은 함정의 재발**(위 "함정(중요)" 항목 참고). 러너를 `EnumColumnConstraintFix` 로 일반화했다.
- **에뮬레이터 화면 깨짐 주의**: Impeller 렌더링이 깨지면 **여러 시점의 프레임이 뒤섞여** 캡처된다(옛 오류 화면과 새 화면이 한 장에 겹쳐 보였다). 이때 화면은 증거로 쓸 수 없으니 **logcat/서버 로그로 판단**할 것. 앱 재시작으로 안 풀리면 `adb reboot`(약 1분)로 해소된다.
...
### 관리자 콘솔도 직거래 단계 반영 (2026-08-08)

- 백엔드 `AdminDto.OrderRow` 에 `paid/cancelled/sellerConfirmed/buyerConfirmed` 추가. 콘솔(`ART_NARA_ADMIN`)은 단계 배지(예약 중→수령 확인 중→결제 대기→거래 완료→예약 취소/환불)를 그리고, **환불 버튼은 `paid` 인 건에만** 보여준다(결제 전엔 "결제 전" 배지 — 돌려줄 돈이 없고, 취소는 앱의 당사자들이 한다). 구버전 서버(paid 필드 없음)면 예전 표시로 동작.
- 서버 가드 실측: 미결제 예약 환불 시도 → `400 ORDER_400_HANDOVER "아직 결제되지 않은 예약입니다. 예약 취소로 처리해주세요."`.
- 대시보드 매출은 `paid && !refunded` 만 집계(₩1,370,000 = 결제 4건, 예약·수령확인 건 제외 확인).
- 콘솔 UI 로 실 PG 건(주문 227) 환불 왕복 실측: 배지 '환불' 전환 → **토스 API 조회로 `CANCELED, balanceAmount 0` 확인** → 작품 잠금 해제 + 소유권 회수 + 구매자 `ORDER_REFUNDED` 알림.
- 콘솔 검증 팁: 브라우저 패널이 안 떠 스크린샷이 불가하면 `javascript_tool` 로 DOM 을 직접 읽어 검증한다(폼 submit 이벤트 디스패치로 로그인 가능).

## 회귀 스위프 (2026-08-08 오후) — 67케이스 전건 통과

직거래 도입 후 전체 재검. `flutter test` 23건 / `./gradlew test` 통과.

- **기본 34**(`sweep.py`): 공개 조회 8 · 비로그인 차단 6 · 신원 스코프 7 · 토큰 1 · 관리자 격리 2 · 하트 2 · QR 4 · 오류 매핑 4.
- **직거래 12**(`handover.py`): 예약(미결제·인증서 없음) · 작품 `reserved` 잠금(sold 아님) · 중복 예약 409 · 확인 전 결제 400 · 한쪽만 확인 시 결제 400 · 제3자 확인 403 · 예약 취소 후 잠금 해제.
- **관리자·정산·경매 21**(`sweep2.py`): 관리자 주문에 단계 필드 노출 · 미결제 예약 환불 거부 400 · 예약해도 매출 불변 · 정산=판매합계-수수료 · 역경매(예산 초과 422 / 정상 200 / 최저가보다 높은 재제안 422) · 경매(증분 미달 422 / 정상 입찰 200 / 마감 전 예약 400) · 앱 토큰으로 관리자 API 403.
- **스위프 짤 때 헛디딘 것**: `/api/artworks` 페이징 응답은 **Spring Page 라 키가 `content`**(피드의 `artworks` 와 다르다) — `artworks` 만 보면 "0건" 으로 오독한다. 그리고 예약 대상 작품을 고를 때 **`auction` 필드로 걸러야** 한다(경매는 마감 전 예약이 막히는 게 정상인데 `auctionEndAt` 로 거르면 목록에 없어 경매 작품을 집는다).

## DB 초기화 후 시드 회귀 (2026-08-08 저녁) — 90케이스 전건 통과

`~/artnara-db` 삭제 → 재기동으로 시드를 새로 만든 상태에서 전부 재검증. 시드는 **작품 8건(전부 판매 전)·회원 8명**으로 깨끗하게 재생성되고 중복이 없다.

- **거래 전 구간 23**(`fullflow.py`): 예약(매출·정산에 안 잡힘) → 작가 전달 확인 → 구매자 수령 확인(결제 대기) → 결제 → 인증서 `ARTNARA-2026-101` → 소유권·QR 유효 → 매출 0→320,000 → 정산 320,000/수수료 32,000/예정 288,000 → 리뷰 자격 → 관리자 환불 → 판매 재개·소유권 회수·QR 무효·매출 감소.
- 기본 34(`sweep.py`) + 직거래 가드 12(`handover.py`) + 관리자·정산·경매 21(`sweep2.py`) 모두 통과. `flutter test` 23 / `./gradlew test` 통과.
- **앱 동작**: 계정이 사라진 상태로 앱을 켜면 `자동 로그인 실패: 401 → 저장 토큰 폐기` 후 온보딩으로 정상 복귀(크래시 없음). 네이버 재로그인 시 웹뷰 세션이 남아 있어 동의까지 자동 통과하며 **새 회원으로 재가입**된다(`isNewUser: true`).
- **시드 사실(스위프 짤 때 반드시 반영할 것)**: `ArtnaraDataInitializer.seedReview` 가 **작품1=Andrew, 작품2=Emma 리뷰를 미리 넣는다** — 그 조합으로 리뷰를 쓰면 409 가 정답이다. 첫 리뷰 작성을 보려면 리뷰 없는 작품(3번 등)으로 거래를 완주해야 한다. 회원 id 도 시드 기준(1 김예진 · 2 박소현 · 3 Andrew · 4 이수민 · 6 Emma)으로 토큰을 만들어야 한다.

## 토큰 폐기 기준선 (2026-08-08) — 로그아웃 404 + 남의 계정에 붙던 토큰

DB 초기화 뒤 5556 을 켜자 **5554 의 계정으로 자동 로그인**되는 걸 실측했다. 옛 토큰의 `sub=9` 가 살아 있었고, 시드를 다시 넣으면서 새 회원이 id 9 를 차지했기 때문. `/auth/refresh` 는 서명·만료·회원 존재만 봤지 **"같은 사람인지"** 는 보지 않았다. 동시에 앱이 부르던 `POST /auth/logout` 은 서버에 없어 **404** 였다(앱이 실패를 감안해 로컬 로그아웃만 했다).

둘 다 `User.tokenValidFrom` 하나로 해결했다 — **이 시각보다 먼저 발급된 토큰은 무효**.

- **가입 시** 기준선을 '지금'으로 잡는다(`startTokenBaseline`) → 그 id 로 예전에 발급됐던 토큰은 거부, 방금 발급할 토큰은 통과.
- **로그아웃 시** '다음 초'로 올린다(`invalidateIssuedTokens`) → 잃어버린 기기의 refresh token 이 즉시 무력화된다. 무상태 JWT 에서 유일한 폐기 수단.
- **`POST /auth/logout` 추가**. `/auth/**` 는 permitAll 이라 access token 이 만료되면 Principal 이 비므로, **body 의 refreshToken 으로도 신원을 찾는다**(서명·만료가 멀쩡한 토큰만 인정 — 아무 문자열로 남을 로그아웃시킬 수 없다).
- **함정(실측)**: 기준선을 '지금'으로 두면 **JWT `iat` 가 초 정밀도라 같은 초에 발급된 토큰이 살아남는다**. 로그아웃 직후 refresh 가 200 으로 통과해서 발견했다. 로그아웃만 `plusSeconds(1)` 로 올려 해결(가입은 '지금' 그대로 — 안 그러면 방금 발급한 토큰이 막힌다).
...
## 밤샘 에뮬레이터 QA (2026-08-09) — 결함 8건 발견·수정

`art_nara`(5554, 네이버 컬렉터 Collector/id 9) + `art_nara2`(5556, 카카오 작가 경원/id 10) 2대로 전 기능 왕복. 통과한 것: 직거래 완주(예약→양쪽 확인→토스 샌드박스 결제→인증서 ARTNARA-2026-133), 경매 실시간(상세를 열어둔 채 상대가 입찰 → 화면 조작 없이 ₩150,000→₩250,000, 마감 시 낙찰자 표시까지 실시간), 역경매(예산 초과 거절→350,000 제안→알림), 채팅+FCM 푸시(도착→탭→방 직행), 리뷰(포트폴리오 평점 3.0/1건), 정산(120,000/수수료 12,000/예정 108,000), QR 인증서 카드, 지도 핀·줌, 관심 작품, 역할 전환, 차단→해제→재로그인.

**고친 결함**

1. **시각이 기기 시간대와 어긋났다** — 서버가 오프셋 없는 `LocalDateTime` 을 그대로 내려, GMT 에뮬레이터에서 01:54(KST)에 보낸 채팅이 "오전 1:54" 로 떴다(기기 시계는 16:54). 운영 서버를 UTC 로 띄우면 실사용자도 9시간 어긋난다. → `TimeZoneConfig`(JVM 기본 시간대 Asia/Seoul 고정 + 응답에 `+09:00` 부착), 앱은 `utils/server_time.dart` 로 파싱 후 `toLocal()`. **Boot 4 는 Jackson 3(`tools.jackson`)이라 모듈이 아니라 `JsonMapperBuilderCustomizer` 로 등록해야 기본 날짜 모듈을 이긴다**(모듈로 붙이면 조용히 덮인다).
2. **판매자가 자기 작품에 입찰·예약할 수 있었다**(자전 거래) — 스스로 값을 올려 낙찰받고 판매 이력·정산을 부풀릴 수 있다. → `BID_403_OWN` / `ORDER_403_OWN`, JWT 신원으로 차단. 상세 응답에 `ownedByViewer` 를 실어 **앱은 입찰 바·문의하기 버튼을 아예 감춘다**.
3. **로그아웃·차단이 이미 발급된 액세스 토큰에 안 먹었다** — 기준선(`tokenValidFrom`) 검사가 `/auth/refresh` 에만 있어, 로그아웃한 토큰으로 프로필·정산이 계속 200 이었고 차단된 계정도 최대 1시간 동안 결제·입찰이 가능했다. → `AppTokenGuard` 를 JWT 필터에 붙여 **요청마다** 확인. 토큰이 실린 요청만 기본키 조회 1회(공개 조회는 토큰이 없어 조회도 없음), 관리자 토큰은 users 와 무관하므로 건너뛴다.
4. **관리자 대시보드 '환불 N건' 이 틀렸다** — `orders.size() - paid.size()` 라 예약·취소까지 환불로 셌다(실제 5건인데 13건). → 환불된 주문만 센다.
5. **예약한 본인에게도 "예약 중인 작품입니다"** 로 떠 남이 채간 것처럼 읽혔다. → `reservedByViewer` 로 '내가 예약한 작품이에요 + 주문 내역' 안내.
6. **의뢰 알림을 눌러도 화면이 그대로였다** — 의뢰 목록이 등록 폼 아래에 있어 탭만 바뀌었다. → 알림의 targetId 를 탭에 넘겨 해당 카드로 스크롤(`onOpenTab(index, {targetId})`).
7. **의뢰 목록 제목이 '내 의뢰 현황'** 인데 실제로는 공개 목록이라 작가 화면에서 남의 의뢰가 자기 것처럼 보였다 → '등록된 의뢰'. 채팅 목록의 본문 제목 중복도 제거(제목은 헤더에서만).
8. **역경매 거절 문구가 첫 제안 때 틀렸다** — 제안이 없으면 상한은 '현재 최저가' 가 아니라 의뢰 예산이다. 금액도 `400000원` → `₩400,000`.

**QA 스크립트 주의**
- `scratchpad/*.py` 는 `jwt41.txt`(컬렉터) / `jwt9.txt`(작가) 를 읽는다. **DB 를 비우거나 로그아웃하면 그 토큰이 무효**가 되어 전 케이스가 401 로 무너진다 — 회원 id 를 확인하고 새로 발급할 것(시크릿은 `application.yml` 기본값, 클레임 `roles=USER`).
- `fullflow.py` 는 시드 작품 2·3 을 쓰다가 두 번째 실행에서 "이미 판매됨" 으로 깨졌다. 지금은 **매번 새 작품을 등록해** 쓰므로 반복 실행해도 통과한다.
- 회귀 89케이스(sweep 34 · sweep2 21 · handover 12 · fullflow 22) 전건 통과, `flutter test` 26 / `./gradlew test` 통과.
- 남은 잔재: dev DB 에 점검용 작품('자전거래 점검용', '자전거래 점검용2', 'QA 야간 …')이 남아 있다. 작품 삭제 API 가 없으므로 정리하려면 `~/artnara-db` 를 지우고 재기동해야 한다.
...
- **활동명(닉네임)을 바꾸면 과거 판매 이력과 끊어진다.** 작품·주문·소유권이 작가를 **활동명 문자열**로 들고 있어서, 마이페이지에서 닉네임을 바꾸면 `GET /api/settlements`(활동명 기준)에서 이전 판매가 사라지고 포트폴리오도 옛 이름으로 남는다.
  - 해결 방향 두 가지: (a) 작품·주문에 작가 **id** 를 저장하고 정산·포트폴리오를 id 기준으로 조회, (b) 닉네임 변경 시 저장된 활동명을 일괄 갱신. (a) 가 정석이지만 화면 다수가 이름 기준이라 범위가 크다 — **사용자 판단 후 진행**.
  - 지금은 아무도 개명하지 않아 실제 데이터 문제는 없다.
